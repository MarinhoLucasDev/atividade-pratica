...
const ListarTarefa = () => {
  ...
  const filteredTarefas = tarefas.filter((tarefa) =>
    ...
    tarefa.statusTarefa.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  />
        ...
        <TableContainer component={Paper} sx={{ borderRadius: '8px', boxShadow: '0px 4px 10px rgba(0, 0, 0, 0.2)' }}>
          <Table sx={{ minWidth: 650, backgroundColor: '#f9f9f9' }} size="small" aria-label="a dense table">
                <TableHead>
                <TableRow>
                    <TableCell>#</TableCell>
                ...
                {filteredTarefas.map((row, indice) => (
                    ...
                    >
                      ...
                      <TableCell align="right" sx={{ color: statusColors[row.statusTarefa] }}>{row.statusTarefa}</TableCell>
                      <TableCell align="right">{row.recursoTarefa}</TableCell>
                      <TableCell align="center">
                        <Button variant="contained" color="success" onClick={() => handleEditar(row.idTarefa)} sx={{marginRight: '8px'}}><EditIcon fontSize="small" /></Button>
                      </TableCell>
                      <TableCell align="center">
        ...
        <CardActions>
            ...
            <Button size="small" variant="outlined" onClick={() => setSearchTerm('')}>Cancelar</Button>
      </CardActions> 
    </Card>
    <div>
      <Modal
